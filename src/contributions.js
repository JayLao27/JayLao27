...
class ContributionsTimeline {
    constructor(containerId) {
        this.contributions = [];
        this.animationSpeed = 5000; // milliseconds per year
        this.boxSize = 12;
        this.boxGap = 3;
    ...
    updateDisplay() {
        ...
        years.forEach((year, index) => {
            ...
            if (!yearSection)
                return;
            const boxesContainer = yearSection.querySelector('.boxes-container');
            if (!boxesContainer)
                return;
            if (index <= this.currentYearIndex) {
                ...
                boxes.forEach(box => {
                    box.style.opacity = '0';
                    box.style.transform = 'translateX(-20px) scale(0.8)';
                });
            }
        });
    }
    addContribution(year, month, day, count, message) {
        this.contributions.push({
            year,
            month,
            day,
...
if (typeof window !== 'undefined') {
    window.addEventListener('DOMContentLoaded', () => {
        ...
        window.contributionsTimeline = timeline;
    });
}
